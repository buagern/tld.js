;(function(){
  ...
  /**
   * Object constructor
   * Convenient for node.js usage; almost useless in the browser
   *
   * @return {tld}
   */
  tld.init = function () {
  ...
  tld.getCandidateRule = function (host, rules) {
    ...
    rules.reverse().some(function(r){
      ...
      if ((new RegExp(pattern)).test(host)){
        rule[r.type || 'normal'] = r;
        return !(r.type === 'exception');
      }
  ...
  tld.prototype.getDomain = function (hostname) {
    ...
    if (rule.normal && rule.exception) {
      rule.normal.type = rule.exception.type;
    }

    if (rule.normal.type === 'exception'){
      pattern = '(' + tld.getExceptionPattern(rule.normal) + ')$';
    }
    else if (rule.normal.type === 'wildcard'){
  ...
  tld.prototype.isValid = function (host){
    return !(typeof host !== 'string' || !~host.indexOf('.') || host[0] === '.');
  };

  /**
   * Returns a `.sld.tld` or `.tld` according to the rule construction
   *
   * @static
   * @param rule {Object}
   * @return {String}
  ...
  /**
   * String pattern helper for regular rule
   *
   * @static
   * @param rule {Object}
   * @return {String} Ready to be used with as a RegExp object parameter
   */
  ...
  /**
   * String pattern helper for wildcard rule
   *
   * @static
   * @param rule {Object}
   * @return {String} Ready to be used with as a RegExp object parameter
  ...
  /**
   * String pattern helper for exception rule
   *
   * @static
   * @param rule {Object}
   * @return {String} Ready to be used with as a RegExp object parameter
